...
import eris.integers.digits;
import eris.integers.exception;
...
//--------------------------------
	...
	unittest {
		...
		xint a, b;
		assertZero(a);
		b = "847224902923494152615773828500";
		assertEqual(b.sqr,b*b);
		b = "8535533905932737622000";
		assertEqual(b.sqr,b*b);
		a = long.min;
...
//--------------------------------
	...
	@safe
	public bool isValidUlong() const {
...
//--------------------------------
	...
	@safe
	private bool opEquals(T:xint)(const T that) const {
...
//--------------------------------
	...
	public ubyte[] toByteArray() const {
		size_t index = 0;
		ubyte[] buffer = new ubyte[digits.length * 4];
		foreach (digit; digits) {
			std.bitmanip.write!size_t(buffer, digit, &index);
		}
		return buffer;
	}
	...
	public this(T:ubyte[])(T buffer) {
		size_t n = buffer.length / 4;
		size_t m = buffer.length % 4;
		if (m != 0) {
			n++;
			buffer.length = n * 4;
		}
...
//		assertEqual(xint("0b11111111111111111111111111111111_11111111111111111111111111111111_11111111111111111111111111111111_11111111111111111111111110000101"), snum);
	...
	public string toString(string fmt = "%s") const {
	...
	public void formatDecimal(scope void delegate(const (char)[]) sink,
		...
		size_t minw = str.length + (signChar ? 1 : 0);
		...
		size_t maxw = minw < f.width ? f.width : minw;
		size_t difw = maxw - minw;
	...
	public string toDecimalString(bool addSign = true) const {
		char[] str;
		uint[] from = this.digits.dup;
		size_t n = numDigits(from);
		if (n == 0) return "0";
		while (n > 0) {
			uint mod;
			char[1] ch;
	...
	public string toBinaryString() const {
		char[] str;
...
//--------------------------------
	...
	@safe
	public xint complement() const {
		auto temp = digits.dup;
		eris.integers.digits.reduce(temp);
	...
	@safe
	public xint toTwosComplement() const {
...
//--------------------------------
	...
	@safe
	private xint opBinary(string op, T:xint)(const T that) const {
		static if (op == "+") {
	...
	@safe
	private xint opBinary(string op, T)(const T that) const {
...
//--------------------------------
	...
	private static xint mul(in xint x, in xint y) {
		uint[] xd;
		size_t nx = copyDigits(x.digits, xd);
		...
		uint[] yd;
		size_t ny = copyDigits(y.digits, yd);
...
//--------------------------------
	...
	@safe
	public bool testBit(int n) const {
		uint d = n / 32;
...
//--------------------------------
	...
	/// zeros, if any.
	@safe
	public size_t getDigitLength() const {
	...
	@safe
	public void reduce() {
		eris.integers.digits.reduce(this.digits);
	}
