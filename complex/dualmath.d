...
//--------------------------------
	...
	public this(const R re, const R hy) {
		this.r = re;
		this.h = hy;
	}

	public this(const R re) {
		this.r = re;
		this.h = 0.0;
	}
...
//--------------------------------
	...
	private const perplex opBinary(string op, T:R)(const T that)
	{
		static if (op == "+") {
			return add(this, that);
		} else static if (op == "-") {
...
writefln("z/w = %s", z/w);
	...
	public static perplex add(const perplex z, const R x) {
		return perplex(z.r+x, z.h);
	}

	/// Subtracts a real number from a perplex number
	/// and returns the difference.
	public static perplex sub(const perplex z, const R x) {
		return perplex(z.r-x, z.h);
	}
	...
	public static perplex mul(const perplex z, const R x) {
...
//		R x = 14;
	...
	{
		R denom = w.r * w.r + w.h * w.h;
		R rterm = z.r * w.r + z.h * w.h;
		R iterm = z.h * w.r - z.r * w.h;
		return perplex(rterm/denom, iterm/denom);
	}

	/// Divides a perplex number by a real number
	/// and returns the quotient.
	public static perplex div(const perplex z, const R x) {
